...
def check(ip, port=445):
    ...
    try:
        ...
        if nt_status == '\x05\x02\x00\xc0':
            ...
            "[+] [{}] is likely VULNERABLE to MS17-010! ({})".format(ip, native_os)            # vulnerable to MS17-010, check for DoublePulsar infection
            ...
            if smb.multiplex_id == 0x0051:
              ...
              output = output + "Host is likely INFECTED with DoublePulsar! - XOR Key: {}".format(key)
...
def run(**args):
    nm = nmap.PortScanner()
    nm.scan(hosts='10.0.0.0/24', arguments='-p 445 --open')
    hosts_list = [(x, nm[x]['status']['state']) for x in nm.all_hosts()]
    outputToSend = ""
    for host, status in hosts_list:
        temp = check(host)
        if temp:
            outputToSend = outputToSend + temp + "\n"
    print "omri:" + outputToSend
    return outputToSend
# run()
